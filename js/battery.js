...
const chargeProgress = document.querySelector('#battery dd:nth-of-type(2) progress');
...
function error() {
    ...
    statusMsg.textContent = "Device OrientationEvent not supported by your browser!";
}
...
if (navigator.getBattery) {
    navigator.getBattery().then(function(battery) {
        ...
        chargeProgress.value = Math.round(battery.level * 100);
        
        // Get the battery percentage
        var batteryPercentage = Math.round(battery.level * 100);
        // Construct the URL for Robohash API using the battery percentage
        ...
        document.getElementById('robohash-img').src = robohashURL;
    });
} else {
    // Handle the case where Battery API is not supported
    error();
}